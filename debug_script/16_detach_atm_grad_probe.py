"""Probe: does stop_gradient on the ATM->OCN forcing fields (cutting the
chaotic-atmosphere feedback path out of the backward pass, forward values
unchanged) stabilize gradients at rollout lengths where the full-coupled
gradient became unreliable (n=20-25, per debug_script/13-15)?

Monkeypatches vercor.setups._external.veros_fluxes.compute_fluxes to wrap
its runtime_fields (the ATM->OCN exchanged fields: u/v velocity, humidity,
model_level_height, density, potential_temperature, temperature, radiation
fluxes) in jax.lax.stop_gradient before Veros' bulk-flux formulas consume
them. This is the exact boundary where atmosphere's chaotic state feeds
into ocean surface forcing each step -- cutting it there means d(loss)/
d(c_k) no longer threads through ATM's dynamics at all, only through OCN's
own repeated forcing-application physics. Forward values are identical
(stop_gradient is the identity function forward); only the backward graph
changes.

Sweeps N_STEPS = [20, 50, 100, 200] (compare 20 against debug_script/15's
un-detached result as a sanity check), single value_and_grad(c_k) + FD at
eps=1e-2 each, stopping early if it also breaks down.
"""
...
print("Monkeypatched compute_fluxes: ATM->OCN forcing fields are now stop_gradient'd.")

C_K_TEST = 0.1
FD_EPS = 1e-2
STEP_SWEEP = [20, 50, 100, 200]
