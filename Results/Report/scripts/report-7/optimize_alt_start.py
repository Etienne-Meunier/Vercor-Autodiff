"""Report 7: same windowed optimization as Report 6 (teacher forcing +
cosine LR decay), but from a different starting point, to test Report 6's
identifiability hypothesis. Report 6 started below true on both params
(c_k=0.05 < 0.1, c_eps=0.4 < 0.7) and converged to c_k=0.169 (above true),
c_eps=0.503 (below true) -- a plausible ridge/correlated-parameters
artifact rather than the true optimum. This run starts from the opposite
corner (both params ABOVE true: c_k=0.15, c_eps=0.9). If it converges to
a similarly-low-loss but DIFFERENT (c_k, c_eps) combination than Report
6's endpoint, that confirms identifiability (a ridge of comparably-good
solutions, not one true optimum being missed by chance) rather than a
one-off optimizer artifact.
"""
...
OUT_DIR = os.path.join(_REPO_ROOT, "Results", "Report", "figures", "report-7")
...
INIT_C_K, INIT_C_EPS = 0.15, 0.9  # opposite corner from Report 6's 0.05, 0.4
...
axs[0].plot(windows, losses, "o-", color="tab:brown")
...
axs[1].plot(cks, ceps, "-o", color="tab:brown", markersize=4, label="optimization path")
axs[1].plot(cks[0], ceps[0], "o", color="black", markersize=8, label="start")
axs[1].plot(cks[-1], ceps[-1], "s", color="tab:brown", markersize=8, label="final")
axs[1].plot(TRUE_C_K, TRUE_C_EPS, "*", color="red", markersize=20, markeredgecolor="black", label="true", zorder=5)
axs[1].plot(0.16942548781633035, 0.5026829251760818, "D", color="tab:purple", markersize=9,
            markeredgecolor="black", label="Report 6 endpoint", zorder=4)
...
fig.suptitle(f"Report 7: windowed calibration from alt start, {N_WINDOWS} windows x {W} steps ({N_WINDOWS*W} simulated days), cosine LR decay")
fig.tight_layout()
