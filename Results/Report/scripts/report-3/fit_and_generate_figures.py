"""Report 3: c_k/c_eps fit over a longer coupled rollout (N_STEPS=20, 2x
report-2's 10 steps), same coupled OCN+LND+ATM(jcm) model.

Preceded by a gradient-vs-finite-difference bracketing pass
(debug_script/13-15) across N_STEPS in [12, 15, 20, 25]: the AD gradient
stays remarkably stable (-4700 to -5700) across all four lengths, and
agrees well with FD at an appropriately-scaled eps=1e-2 (7-27% relative
error) at n=12/15/20 -- the eps=1e-3 FD estimate is garbage at every
length tested (chaotic amplification of a too-small perturbation, not a
sign the AD gradient itself is wrong -- see debug_script/14's eps sweep).
n=25 is the one length where even eps=1e-2 FD disagreed badly (350% rel
err) -- close to where trustworthiness likely breaks down, but not
re-verified with a matching eps sweep. n=20 was chosen for this report as
the largest length with solid gradient-vs-FD agreement.

Runs the fit itself (like report-2), producing the same two figures
...
OUT_DIR = os.path.join(_REPO_ROOT, "Results", "Report", "figures", "report-3")
os.makedirs(OUT_DIR, exist_ok=True)

N_STEPS = 20  # 2x report-2's 10 steps
N_ITERS = 8   # fewer than report-2's 10 -- each iteration costs ~2x as long
LEARNING_RATE = 2e-2
...
fig.suptitle(f"Report 3: c_k/c_eps recovery, {N_STEPS}-step coupled OCN+LND+ATM(jcm) rollout")
fig.tight_layout()
...
for ax in axs:
    ax.set_xlabel("xt")
axs[0].set_ylabel("yt")
fig.suptitle(f"Report 3: temperature bias after {N_STEPS} steps, surface level (global_4deg_learning + jcm)")
